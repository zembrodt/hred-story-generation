...
def convertLinesToParagraphs(lines):
    print('convertLinesToPara: lines: {}'.format(len(lines)))
    paragraphs = []
    curr_paragraph = []
    for line in lines:
        if len(line.strip()) > 0:
...
def main():
    ...
    if os.path.isdir(arg):
        ...
        for book_file in os.listdir(arg):
            ...
            with open(book_file_path, encoding='utf-8', errors='ignore') as f:
                lines = f.read().strip().split('\n')
            paragraphs = convertLinesToParagraphs(lines)
            ...
            wordcount = Counter(list(itertools.chain.from_iterable( \
                list(itertools.chain.from_iterable(paragraphs)))))
            # TODO: potentially add filter for lowest count of words
            dict_data = [('<SOS>', 0), ('<EOS>', 1)]
            i = 2
            ...
            train_valid_size = int(0.8*len(paragraphs))
            train_valid_paragraphs = paragraphs[:train_valid_size]
            train_size = int(0.8*len(train_valid_paragraphs))

            train_paragraphs = train_valid_paragraphs[:train_size]
            ...
            num_sentences[book_file] = len([sentence for sentence in sentences if len(sentence.strip()) > 0])
            num_paragraphs[book_file] = len(paragraphs)
            # Saved parsed file
        ...
        for book in num_paragraphs:
            print('\t{}: {}'.format(book, num_paragraphs[book]))
    else:
        print('{} is not a directory!'.format(arg))
